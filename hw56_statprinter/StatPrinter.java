...
public class StatPrinter {
  ...
  //O(n) because it would be O(2n) due to two separate for loops but we ignore constants
  public StatPrinter( ArrayList <Integer> data )
  {
    ...
    for(int i = 0; i < max(data) + 1; i++){
      _frequency.add(0);
      // System.out.println(_frequency.get(i));
    }
  ...
  //    isLocalMode(1) -> true
  //    isLocalMode(5) -> true
  //O(1) - constant time doesn't matter number of elements
  ...
  //O(n) - increases with size of frequency lineraly
  public ArrayList<Integer> getLocalModes()
  {
    ArrayList<Integer> result = new ArrayList<Integer>();
    for(int i = 0; i < _frequency.size(); i++){
      //adding local modes to result
      if(this.isLocalMode(i)){
  ...
  {
    String bar;
    //find scale so bars are in proportion to longestBar
    int scale = longestBar / max(_frequency);
    for(int i = 0; i < _frequency.size(); i++){
      bar = "";
      for(int j = 0; j < _frequency.get(i) * scale; j++){
        bar += "*";
